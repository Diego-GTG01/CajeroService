...
public class UsuarioDAOImplementation implements IUsuario {
    ...
    public Result<Usuario> createUsuario(String email, String password, String numTarjeta) {
        Result<Usuario> result = new Result<>();
        try {
            Result<Usuario> existe = getUsuarioByEmail(email);
            if (existe.correct) {
                result.correct = false;
                result.message = "Ya existe un usuario con el email: " + email;
                return result;
            }
            
            String sql = "INSERT INTO USUARIO (nombre, apellidoPaterno, apellidoMaterno, celuar, telefono, email) " +
                        "VALUES ('Nuevo', 'Usuario', 'Sistema', '0000000000', '0000000000', ?)";
